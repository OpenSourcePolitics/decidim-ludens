...
module Decidim
  describe User do
    ...
    describe "#ludens_enabled?" do
      context "when the user has not changed his settings" do
        it "returns true" do
          expect(subject.ludens_enabled?).to eq(Decidim::Ludens.enable_ludens)
        end
      end

      context "when the user has changed his settings" do
        before do
          subject.update!(enable_ludens: !Decidim::Ludens.enable_ludens)
        end
