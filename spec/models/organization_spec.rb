...
module Decidim
  describe Organization do
    ...
    describe ".increase_score" do
      it "returns the increased score" do
    ...
    describe ".lastDoneRecommendations" do
      let!(:participative_action) { create(:participative_action, organization: subject) }

      before do
        subject.assistant["last"] = participative_action.id
      end
    ...
    describe ".recommendations" do
      context "when there's just one of each points" do
        ...
        let!(:participative_actions2) { create_list(:participative_action, 1, points: 1, organization: organization2) }
      ...
      context "when there are multiple ones" do
        let!(:participative_action1) { create(:participative_action, points: 1, organization: subject) }
        let!(:participative_action2) { create(:participative_action, points: 1, organization: subject) }
      ...
      context "when participative actions are completed" do
        let!(:participative_actions) { create_list(:participative_action, 5, :completed, points: 1, organization: subject) }
